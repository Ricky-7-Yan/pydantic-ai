...
async def test_openai_responses_compact_emits_lifecycle_events(allow_model_requests: None, openai_api_key: str):
    """Stateless `OpenAICompaction` emits an inline `CompactionStartEvent` and a `CompactionEndEvent` around `/compact`."""
    model = OpenAIResponsesModel('gpt-4o-mini', provider=OpenAIProvider(api_key=openai_api_key))
    agent = Agent(
        model=model,
        instructions='You are a helpful math assistant. Give short answers.',
    ...
    assert compaction_events == snapshot(
        [
            CompactionStartEvent(capability_id='open_ai_compaction', strategy='OpenAICompaction', message_count=4),
            CompactionEndEvent(
                capability_id='open_ai_compaction', strategy='OpenAICompaction', messages_before=4, messages_after=1
            ),
        ]
    )
