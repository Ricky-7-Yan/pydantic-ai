...
async def test_openai_responses_compact_emits_lifecycle_events(allow_model_requests: None, openai_api_key: str):
    ...
    assert compaction_events == snapshot(
        [
            CompactionStartEvent(capability_id='open_ai_compaction', strategy='openai', message_count=5),
            CompactionEndEvent(
                capability_id='open_ai_compaction', strategy='openai', messages_before=5, messages_after=2
            ),
        ]
    )
