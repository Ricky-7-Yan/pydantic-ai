...
The namespace and event name form the serialized `kind` (for example, `file_system.file_read`). Kinds are registered when the class is defined and must be unique within the process; re-executing the same class definition (as when re-running a notebook cell) replaces the registration. Import the module defining an event before deserialization to recover its typed subclass. Otherwise it becomes [`UnknownCapabilityEvent`][pydantic_ai.messages.UnknownCapabilityEvent] without losing payload fields; serializing it again preserves the wire representation so a later consumer can recover the typed event.
